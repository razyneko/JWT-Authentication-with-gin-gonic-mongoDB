...
func SignUp() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		// checking if enetered email already exists
		count, err := userCollection.CountDocuments(ctx, bson.M{"email": user.Email})
		defer cancel()
		if err != nil {
			log.Panic(err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error ocurred while checking for email"})
		}

		password := HashPassword(*user.Password)
		user.Password = &password
		
		// checking if enetered phone already exists
		count, err = userCollection.CountDocuments(ctx, bson.M{"phone": user.Phone})
		if err != nil {
			log.Panic(err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error ocurred while checking for phone"})
		}
		if count > 0 {
...
// this function can only be used by an admit to get all entries
func GetUsers() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// pipeline stage functions / aggeregation functions of mongodb
		...
		// $sum for count of users,  $push $root to get all data of users

		projectStage := bson.D{
			// helps to define which data points are to be sent to user
...
// only admin will have the acces to users' data
